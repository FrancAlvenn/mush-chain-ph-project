...
contract MushChainPH {
    ...
    /**
     * @notice Returns the FULL audit trail for a batch in a single call.
     *         Every field of every audit entry is returned as parallel arrays —
     *         index [0] = first event, index [1] = second event, etc.
     *
     *         In Remix, the decoded output will show 6 labeled arrays:
     *           entryIds   — the internal audit entry IDs
     *           actions    — e.g. "BATCH_REGISTERED", "OWNERSHIP_TRANSFERRED"
     *           actors     — wallet address of who performed each action
     *           actorRoles — "Farmer", "Trader", "Transporter", "Vendor"
     *           details    — full human-readable description of what happened
     *           timestamps — unix timestamp of each event
     *
     *         You can read all 4 supply chain events for a batch at once,
     *         with no need to call getAuditEntry() separately.
     */
    function getBatchAuditTrail(uint256 _batchId)
        ...
        returns (
            ...
            string[]  memory actorRoles,
            string[]  memory details,
            uint256[] memory timestamps
        )
    {
        uint256[] storage ids = batchAuditTrail[_batchId];
        uint256 count = ids.length;

        entryIds   = new uint256[](count);
        actions    = new string[](count);
        actors     = new address[](count);
        actorRoles = new string[](count);
        details    = new string[](count);
        timestamps = new uint256[](count);

        for (uint256 i = 0; i < count; i++) {
            AuditEntry storage e = auditLog[ids[i]];
            entryIds[i]   = e.entryId;
            actions[i]    = e.action;
            actors[i]     = e.actor;
            actorRoles[i] = e.actorRole;
            details[i]    = e.details;
    ...
    /**
     * @notice Returns a single audit log entry by its entry ID.
     *         Use this if you want to inspect one specific event.
     */
    ...
    /**
     * @notice Returns a fully formatted, human-readable audit report string
     *         for a batch — all events concatenated into one readable block.
     *         Great for a quick copy-paste summary or presentation demo.
     *
     *         Example output for a completed batch:
     *         "
     *         [1] BATCH_REGISTERED | Role: Farmer | Actor: 0xAB..cd
     *              Farm: Dela Cruz Mushroom Farm | Variety: Oyster | Qty: 50 kg | ...
     *              Time: 1746000000
     *         ──────────────────────────────────────────
     *         [2] OWNERSHIP_TRANSFERRED | Role: Farmer | Actor: 0xAB..cd
     *              From Farmer: 0xAB..cd -> Trader: 0xCD..ef | Price/kg: 85 | ...
     *              Time: 1746000060
     *         ..."
     */
    function getAuditReport(uint256 _batchId)
        external
        view
        batchExists(_batchId)
        returns (string memory report)
    {
        ...
        report = string(abi.encodePacked(
            ...
            "  Batch: ", b.batchCode, "\n",
            "  Status: ", _statusName(b.status), "\n",
            "  Total Events: ", _uint2str(count), "\n",
            "========================================\n"
        ));
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            report = string(abi.encodePacked(
                report,
                "\n[", _uint2str(i + 1), "] ", e.action, "\n",
                "    Role    : ", e.actorRole, "\n",
                "    Actor   : ", _addrShort(e.actor), "\n",
                "    Details : ", e.details, "\n",
                "    Time    : ", _uint2str(e.timestamp), "\n",
                "----------------------------------------\n"
            ));
        }
    }
